...
8. **警惕并处理极端值**——在建模前，检查变量是否有大量聚集在边界的值（如0值），然后进行剔除。若不剔除，这些边界值会压缩到变换空间的一个点（如 log(1)=0），系统性地扭曲残差结构（残差 vs 拟合图出现非对称尾、Q-Q 图左尾严重偏离），使整体模型诊断不可靠。
9. **避免变量分类过粗**——在研究具有明确特定效应的分类变量（如“国籍”）时，不要简单地切分，挑出具有代表性的单一类别进行对比。
## Skill 索引(按需读取)
